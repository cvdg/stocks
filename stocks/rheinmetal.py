...
def extract_rheinmetal(connection, start: str, end: str) -> bool:
    if start == end:
        return True

    key_end = "rheinmetal-end-date"
    data = yf.Ticker("RHM.DE")    
    ...
    if rows:
        ...
        db_journal_info(f"Fetched RHEINMETAL to: {end} - rows: {rows}")
        logger.info(f"Fetched RHEINMETAL to: {end} - rows: {rows}")
    else:
        db_journal_info(f"Fetched RHEINMETAL no rows")
        logger.info(f"Fetched RHEINMETAL no rows")
    return rows <= 0


def rheinmetal(connection) -> None:
    stop = False
    key_start = "rheinmetal-start-date"
    key_end = "rheinmetal-end-date"
    ...
    while not stop:
        ...
        stop = extract_rheinmetal(connection, string_start, string_end)
